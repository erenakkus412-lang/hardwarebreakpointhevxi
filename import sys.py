...
def get_linux_build_cmd(output_name):
    """Linux ortamında cross‑compile ile Windows exe üretir."""
    return (
        f'x86_64-w64-mingw32-g++ -std=c++23 -o {output_name} breakpointhevxistealer.cpp '
        ...
        f'-lpsapi -lshlwapi -lshell32 -lbcrypt -lsqlite3 -lzip -lcurl -lntdll -static -O2'
    )

def get_windows_build_cmd(output_name):
    """Windows ortamında native MinGW ile exe üretir."""
    return (
        f'g++ -std=c++23 -o {output_name} breakpointhevxistealer.cpp '
        f'NullGate/src/nullgate/syscalls.cpp '
        f'NullGate/src/nullgate/obfuscation.cpp '
        f'NullGate/src/nullgate/syscalls.S '
        f'-I"{PROJECT_ROOT}/NullGate/include" '
